...
class SoritesServiceTest {
    ...
    void bucketize_basic() {
        ...
        ArrayNode result = service.bucketize(values, 5, 5000);
        assertEquals(5, result.size());
        for (int i = 0; i < 5; i++) {
            assertEquals(1, result.get(i).get("count").asInt());
        }
    }

    @Test
    void bucketize_empty() {
        ArrayNode result = service.bucketize(Collections.emptyList(), 5, 5000);
        assertEquals(5, result.size());
        for (int i = 0; i < 5; i++) {
            assertEquals(0, result.get(i).get("count").asInt());
        }
    }
    ...
    void bucketize_edge_values() {
        ...
        ArrayNode result = service.bucketize(values, 10, 10000);
        assertEquals(10, result.size());
        assertEquals(1, result.get(0).get("count").asInt());
        assertEquals(1, result.get(9).get("count").asInt());
    }

    @Test
    void bucketize_labels_correct() {
        ArrayNode result = service.bucketize(Collections.emptyList(), 3, 300);
        assertEquals("0-100", result.get(0).get("label").asText());
        assertEquals("100-200", result.get(1).get("label").asText());
        assertEquals("200-300", result.get(2).get("label").asText());
    }

    @Test
    void bucketize_all_in_one_bucket() {
        List<Integer> values = Arrays.asList(50, 60, 70, 80, 90);
        ArrayNode result = service.bucketize(values, 10, 1000);
        assertEquals(5, result.get(0).get("count").asInt());
        for (int i = 1; i < 10; i++) {
            assertEquals(0, result.get(i).get("count").asInt());
        }
    }
