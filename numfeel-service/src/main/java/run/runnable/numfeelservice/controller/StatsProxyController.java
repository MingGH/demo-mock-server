...
import run.runnable.numfeelservice.controller.dto.UtilityResponses.StatsProxyGetAllResponse;
import run.runnable.numfeelservice.controller.dto.UtilityRequests.StatsProxyQuery;
...
public class StatsProxyController {
    ...
    public Mono<ResponseEntity<JsonNode>> getStats(@ModelAttribute StatsProxyQuery query) {
        return dispatch(query == null ? null : query.action(),
                query == null ? null : query.key(),
                query == null ? null : query.n());
    }

    @PostMapping(value = "/stats", produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<ResponseEntity<JsonNode>> postStats(@ModelAttribute StatsProxyQuery query) {
        return dispatch(query == null ? null : query.action(),
                query == null ? null : query.key(),
                query == null ? null : query.n());
    }
    ...
    private Mono<ResponseEntity<JsonNode>> handleGetAll() {
        return Mono.zip(
                ...
                .map(tuple -> ApiResponse.ok(new StatsProxyGetAllResponse(
