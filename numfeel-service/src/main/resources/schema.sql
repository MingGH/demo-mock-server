...
-- props 本应使用 JSON 列，但项目内现有 r2dbc-mysql（1.4.2）绑定 JSON 类型缺乏先例验证，
-- 为避免引入风险，这里降级为 TEXT，写入/读取时在应用层手动做 Jackson 序列化。
...
CREATE TABLE IF NOT EXISTS demo_events (
    ...
    props       TEXT        NULL     COMMENT '事件属性 JSON 字符串，扁平结构，仅数值/布尔/短枚举',
