...
(function () {
  ...
  var DEF = D.BAYES_DEFAULTS || { prior: 0.01, likelihood: 0.52, falseRate: 0.32 };
  ...
  var PLAIN_ARTICLE = [
    ...
    '',
    '样本                    | 纹身率 | 来源',
    '美国男性囚犯（成年）    | 52%    | Bales 2013',
    '伊朗囚犯（终身）        | 44.7%  | Jafari 2020',
    '印度 Khandwa 监狱       | 83.9%  | Kaithwas 2022',
    '中国 F 省未管所男犯     | 64%    | 成都未管所 2022',
    '中国 S 市涉罪未成年人   | 50.5%  | 成都未管所 2022',
    '中国成都未管所未成年犯 | 83%    | 成都未管所 2022',
    '',
    '美国普通成人纹身率为 32%（Pew 2023）。监狱数据的极差从 44.7% 到 83.9%，说明这一相关在不同法律体系、不同性别比例下都成立。',
    '',
    ...
    '',
    'Dzhansarayeva 2023 使用 Add Health 纵向数据（N=20,745），控制自控力、越轨同伴接触、人口学变量后，得到有纹身者的司法处理优势比（Odds Ratio）：',
    '',
    '            | 被捕   | 被定罪 | 被监禁',
    '男性        | ×2.50  | ×1.80  | ×2.00',
    '女性        | ×1.75  | ×1.68  | ×1.90',
    ...
    '- 中国强制隔离戒毒所 995 例：男戒员纹身率 33.1%，女戒员 25.6%。',
    '',
    ...
    '- 似然 P(纹身|犯罪) = 52%（美国男性囚犯纹身率）',
    '- 边际 P(纹身) ≈ 0.32 × 0.99 + 0.52 × 0.01 ≈ 0.322',
    '',
    '得到：',
    'P(犯罪|纹身) = (0.52 × 0.01) / 0.322 ≈ 1.61%',
    '',
    '即一个随机抽到的纹身者，真正犯罪的概率约为 1.6%。',
    '',
    '把罪犯纹身率拉到成都未管所水平（83%）：',
    'P(犯罪|纹身) = (0.83 × 0.01) / (0.83 × 0.01 + 0.32 × 0.99) ≈ 2.60%',
    '',
    '仍然在 3% 以内。',
    '',
    ...
    '六、回到题目',
    '',
    '"纹身泡吧=坏女孩"这个印象，有数据基础——纹身在统计学上关联了更高的事务处理概率。这个关联值得作为风险信号保留。',
    '',
