...
import run.runnable.numfeelservice.controller.dto.GameplayRequests.StroopSubmitRequest;
...
public class StroopStatsController {
    ...
    @PostMapping("/submit")
    public Mono<ResponseEntity<JsonNode>> submit(@RequestBody(required = false) StroopSubmitRequest request) {
        if (request == null) {
            throw ApiException.badRequest("Invalid JSON");
        }
        Integer total = request.total();
        Integer correctCount = request.correctCount();
        Double accuracy = request.accuracy();
        Double avgRT = request.avgRT();
        Double conAvgRT = request.conAvgRT();
        Double incAvgRT = request.incAvgRT();
        Double stroopEffect = request.stroopEffect();
