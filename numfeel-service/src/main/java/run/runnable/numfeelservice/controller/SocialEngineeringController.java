...
import run.runnable.numfeelservice.controller.dto.TrackingRequests.SocialEngineeringQuestionItem;
import run.runnable.numfeelservice.controller.dto.TrackingRequests.SocialEngineeringSubmitRequest;
...
public class SocialEngineeringController {
    ...
    public Mono<ResponseEntity<JsonNode>> submit(
            @RequestBody(required = false) SocialEngineeringSubmitRequest request) {
        if (request == null) {
        ...
        Integer total = request.total();
        Integer correct = request.correct();
        ...
        List<SocialEngineeringQuestionItem> rawQuestions = request.questions();
        ...
        for (int i = 0; i < Math.min(rawQuestions.size(), MAX_QUESTIONS); i++) {
            SocialEngineeringQuestionItem q = rawQuestions.get(i);
            if (q == null) {
                continue;
            }

            Integer qId = q.questionId();
            String tactic = q.tactic() == null ? "" : q.tactic();
            Boolean isFake = q.isFake();
            Boolean qCorrect = q.correct();
