...
import ninja._6.numfeelservice.service.InceptionMazeService;
...
public class InceptionMazeController {
    ...
    public Mono<ResponseEntity<JsonNode>> submit(@RequestBody(required = false) JsonNode body) {
        ...
        Integer gridSize = Json.getInteger(body, "gridSize");
        Integer pathLength = Json.getInteger(body, "pathLength");
        Integer minPath = Json.getInteger(body, "minPath");
        Double detour = Json.getDouble(body, "detourRatio");
        Integer level = Json.getInteger(body, "dreamLevel");
        Integer wallCount = Json.getInteger(body, "wallCount");
