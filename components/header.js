(function() {
  ...
  var isHome = currentPage === 'index.html' && !pathname.includes('/pages/');
  ...
  if (pagesIdx !== -1) {
    ...
    var depth = afterPages.split('/').length; // 1 = 直接在pages下, 2 = 子目录
    prefix = depth >= 2 ? '../../' : '../';
  }
  var accentMap = {
    probability: '别赌直觉',
  ...
  function loadAssets() {
    ...
    // 按需加载 Chart.js，供页面调用: loadChartJS().then(function() { new Chart(...) })
    var _chartPromise;
  ...
  function boot() {
    ...
    if (isHome) {
      createHomeShell();
      // index.html 的分类数据由 fetch 异步加载，等渲染完成后再增强
      waitForCategories(function() { enhanceHomePage(); });
    } else {
      createDetailShell();
      enhanceDetailPage();
      buildRelatedFooter();
  ...
  function ensureScript(flagName, options) {
    ...
    Object.entries(options).forEach(function(entry) {
      ...
      var value = entry[1];
      if (key === 'dataset') {
        Object.entries(value).forEach(function(dEntry) {
          script.dataset[dEntry[0]] = dEntry[1];
        });
        return;
      }
  ...
  function createHomeShell() {
    ...
    var hostParent = targets[0].parentNode;
    var sameParent = targets.every(function(node) { return node.parentNode === hostParent; });
    if (!sameParent) {
      return;
    }

    var stage = buildStageShell();
    hostParent.insertBefore(stage, targets[0]);
    var content = stage.querySelector('.site-content');
  ...
  function buildHeader() {
    ...
    var navHtml = navLinks.map(function(link) {
      ...
      return '<a href="' + link.href + '"' + activeClass + ' ' + attrs + '>' +
  ...
  function waitForCategories(callback) {
    ...
    if (container && container.querySelector('.category-section')) {
      callback();
      return;
    }
    ...
    // 兜底：3秒后无论如何执行
    setTimeout(function() { observer.disconnect(); callback(); }, 3000);
  }
  ...
  function enhanceHomePage() {
    ...
    var totalValue = total ? total.textContent : '73';
    ...
    var lead = document.createElement('section');
  ...
  function enhanceDetailPage() {
    var rail = document.querySelector('.site-rail');
    var container = document.querySelector('.site-content > .container');
    ...
    if (header && !header.querySelector('.header-sidekick')) {
      var sidekick = document.createElement('div');
      sidekick.className = 'header-sidekick';
      sidekick.innerHTML = '\
        <div class="story-chips">' +
          getStoryChips(titleText).map(function(chip) { return '<span class="story-chip">' + chip + '</span>'; }).join('') +
        '</div>\
  ...
  function getStoryChips(titleText) {
    ...
    if (/随机|概率|量子|彩票|骰子|泊松|蒙提|贝叶斯/i.test(titleText)) {
      return base.concat(['直觉常翻车']);
    }
    if (/订阅|股票|退休|财富|基金|交易|复利|工资/i.test(titleText)) {
      return base.concat(['钱会装样子']);
    }
    if (/热词|同步|异步|SSH|jQuery|云图|缩放/i.test(titleText)) {
      return base.concat(['热闹未必真']);
    }
    return base.concat(['结论别端着']);
  }
  ...
  function shareTo(platform) {
    ...
    var shareUrl = '';
    var features = 'width=640,height=560';
  ...
  function copyShareUrl() {
    var input = document.getElementById('shareUrlInput');
    var value = input && input.value ? input.value : window.location.href;
  ...
  function buildRelatedFooter() {
    var jsonUrl = prefix + 'data/demos.json';
    fetch(jsonUrl)
      .then(function(r) { return r.json(); })
      .then(function(data) {
        var currentFile = currentPage; // e.g. benfords-law.html
        // 子目录页面的路径片段，如 'pages/sample-inference/'
        // 去掉 index.html 以匹配 demos.json 中以 / 结尾的 href
        var currentHrefSuffix = pathname.indexOf('/pages/') !== -1
          ? pathname.slice(pathname.indexOf('/pages/') + 1).replace(/index\.html$/, '')
          : '';
        var currentCat = null;
        var allDemos = [];
        ...
        data.categories.forEach(function(cat) {
          cat.demos.forEach(function(demo) {
            var file = demo.href.split('/').pop();
            allDemos.push(Object.assign({}, demo, { catId: cat.id, catName: cat.name, file: file }));
            // 优先完整路径匹配（子目录），fallback 文件名匹配
            var matched = currentHrefSuffix
        ...
        // 排除当前页
        var others = allDemos.filter(function(d) {
          var hrefSuffix = pathname.indexOf('/pages/') !== -1
            ? pathname.slice(pathname.indexOf('/pages/') + 1).replace(/index\.html$/, '')
            : '';
          if (hrefSuffix) {
        ...
        var sameCat  = shuffle(others.filter(function(d) { return d.catId === currentCat; }));
        var diffCat  = shuffle(others.filter(function(d) { return d.catId !== currentCat; }));
