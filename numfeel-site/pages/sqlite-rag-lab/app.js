...
(function() {
  ...
  const els = {
    ...
    ftsRanking: $('fts-ranking'),
    vecRanking: $('vec-ranking'),
    timingWrap: $('timing-chart-wrap'),
    timingCanvas: $('timing-chart'),
    timingCanvasWrap: $('timing-canvas-wrap'),
    timingSummary: $('timing-summary'),
    timingValues: $('timing-values'),
    timingFallback: $('timing-fallback'),
  ...
  async function ask(question) {
    ...
    els.searchExplanation.hidden = true;
    els.timingWrap.hidden = true;
    els.askStatus.hidden = false;
    els.askTimer.textContent = '0ms';
    ...
    try {
      ...
      if (request === askRequest) {
        els.askBtn.disabled = false;
        els.askStatus.hidden = true;
      }
    }
  }
  ...
  function renderAsk(data, elapsed) {
    renderRanking(els.ftsRanking, data.channels?.fts || []);
    renderRanking(els.vecRanking, data.channels?.vec || []);
    els.searchExplanation.hidden = false;
    const rows = L.describeResultRanks(data.results, data.channels).map((r, index) => {
      const both = r.engines.includes('fts') && r.engines.includes('vec');
      const chip = both
        ? '<span class="chip both">两边都看上它 + RRF 融合</span>'
        : r.engine === 'fts'
          ? '<span class="chip fts">关键词命中</span>'
          : '<span class="chip vec">向量命中</span>';
      ...
      return `
        <div class="result-card">
          <div class="rc-head">
            <span class="rc-title">第 ${index + 1} 名 · ${escapeHtml(r.title)}</span>
            ${chip}
          </div>
          <div class="rc-content">${parts}</div>
          <p class="rank-detail">文本块 ${escapeHtml(r.chunkId)} · 关键词${rankLabel(r.ftsRank)} · 向量${rankLabel(r.vecRank)} · RRF ${Number(r.rrf).toFixed(5)}</p>
        </div>`;
    }).join('');
    els.askResults.innerHTML = rows || '<p class="chart-note">没有检索到原文，换个问题再试。</p>';
    els.timingSummary.textContent = `服务端总耗时 ${L.formatMs(data.totalMs)} · 本次请求往返 ${L.formatMs(elapsed)}`;
    els.timingWrap.hidden = false;
  ...
  function rankLabel(rank) {
    return rank === null ? '未入选' : `第 ${rank} 名`;
  }

  function renderRanking(container, hits) {
    container.innerHTML = hits.length ? hits.map(hit => `
      <li>
        <b>${escapeHtml(hit.title)}</b>
        <span class="rank-detail">文本块 ${escapeHtml(hit.chunkId)}</span>
        <p>${escapeHtml(hit.content)}</p>
      </li>
    `).join('') : '<li class="ranking-empty">本通道没有命中。</li>';
  }
  ...
  function renderTimingChart(timings) {
    ...
    els.timingValues.innerHTML = entries.map(entry => `
      <div><dt>${entry.label}</dt><dd>${L.formatMs(entry.value)}</dd></div>
    `).join('');
    els.timingFallback.hidden = true;
    els.timingCanvasWrap.hidden = false;
    // Keep the measured values available even if the CDN or canvas fails.
    try {
      ...
      els.timingCanvasWrap.hidden = true;
      els.timingFallback.hidden = false;
    }
  }
  ...
  async function feed() {
    ...
    try {
      ...
      els.feedMsg.textContent = `已写入，用时 ${L.formatMs(data.ingestMs)}。回到上面，用文中的关键词试着检索。`;
      els.feedTitle.value = '';
      els.feedText.value = '';
      try {
