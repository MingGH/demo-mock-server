package com.example.demo_mock_server.handler;

import com.example.demo_mock_server.service.SoritesService;
import io.vertx.core.Handler;
import io.vertx.core.json.JsonObject;
import io.vertx.ext.web.RoutingContext;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * 沙堆悖论 HTTP 处理器
 * <p>
 * POST /sorites/submit — 提交实验结果
 * GET  /sorites/stats  — 查询全局统计
 */
public class SoritesHandler implements Handler<RoutingContext> {

    private static final Logger log = LoggerFactory.getLogger(SoritesHandler.class);

    private final SoritesService service;

    public SoritesHandler(SoritesService service) {
        this.service = service;
    }

    @Override
    public void handle(RoutingContext ctx) {
        String method = ctx.request().method().name();
        String path = ctx.request().path();

        if ("POST".equals(method) && path.endsWith("/submit")) {
            handleSubmit(ctx);
        } else if ("GET".equals(method) && path.endsWith("/stats")) {
            handleStats(ctx);
        } else {
            ctx.response().setStatusCode(404).end();
        }
    }

    private void handleSubmit(RoutingContext ctx) {
        JsonObject body = ctx.body() != null ? ctx.body().asJsonObject() : null;
        if (body == null) { sendError(ctx, 400, "Invalid JSON"); return; }

        Integer sandBoundary = body.getInteger("sandBoundary");
        String sandSharpness = body.getString("sandSharpness");
        Integer baldBoundary = body.getInteger("baldBoundary");
        Integer colorBoundary = body.getInteger("colorBoundary");

        if (sandBoundary == null || sandBoundary < -1 || sandBoundary > 10000) {
            sendError(ctx, 400, "invalid sandBoundary"); return;
        }
        if (sandSharpness == null || sandSharpness.isEmpty()) {
            sendError(ctx, 400, "invalid sandSharpness"); return;
        }
        if (baldBoundary == null || baldBoundary < 0 || baldBoundary > 100000) {
            sendError(ctx, 400, "invalid baldBoundary"); return;
        }
        if (colorBoundary == null || colorBoundary < 0 || colorBoundary > 100) {
            sendError(ctx, 400, "invalid colorBoundary"); return;
        }

        service.submit(sandBoundary, sandSharpness, baldBoundary, colorBoundary)
            .onSuccess(v -> ok(ctx, new JsonObject().put("submitted", true)))
            .onFailure(err -> {
                log.error("sorites submit error", err);
                sendError(ctx, 500, "Internal error");
            });
    }

    private void handleStats(RoutingContext ctx) {
        service.stats()
            .onSuccess(data -> ok(ctx, data))
            .onFailure(err -> {
                log.error("sorites stats error", err);
                sendError(ctx, 500, "Internal error");
            });
    }

    private void ok(RoutingContext ctx, JsonObject data) {
        ctx.response()
            .putHeader("Content-Type", "application/json")
            .end(new JsonObject().put("status", 200).put("data", data).encode());
    }

    private void sendError(RoutingContext ctx, int status, String msg) {
        ctx.response()
            .setStatusCode(status)
            .putHeader("Content-Type", "application/json")
            .end(new JsonObject().put("status", status).put("message", msg).encode());
    }
}
