...
import run.runnable.numfeelservice.controller.dto.GameplayRequests.CascadeFailureSubmitRequest;
...
public class CascadeFailureController {
    ...
    @PostMapping("/submit")
    public Mono<ResponseEntity<JsonNode>> submit(@RequestBody(required = false) CascadeFailureSubmitRequest request) {
        if (request == null) {
            throw ApiException.badRequest("Invalid JSON");
        }
        String topology = request.topology() == null ? "" : request.topology().trim();
        int coupling = request.coupling() == null ? 0 : request.coupling();
        int capacity = request.capacity() == null ? 0 : request.capacity();
        String strategy = request.strategy() == null ? "" : request.strategy().trim();
        String triggerPos = request.triggerPos() == null ? "" : request.triggerPos().trim();
        Double survivalRate = request.survivalRate();
        Integer cascadeSteps = request.cascadeSteps();
        Integer maxComponent = request.maxComponent();
        Integer totalNodes = request.totalNodes();
