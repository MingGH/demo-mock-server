...
import run.runnable.numfeelservice.controller.dto.GameplayRequests.WinningStrategySubmitRequest;
...
public class WinningStrategyStatsController {
    ...
    public Mono<ResponseEntity<JsonNode>> submit(
            @RequestBody(required = false) WinningStrategySubmitRequest request) {
        if (request == null) {
            throw ApiException.badRequest("Invalid JSON");
        }
        String game = request.game() == null ? "" : request.game();
